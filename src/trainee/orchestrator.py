...
class RuntimeService:
    ...
    def _launch_project_spec(self) -> ProjectSpec:
        ...
        return ProjectSpec(
            ...
            launcher_template=self._default_launcher_template(),
        )
    ...
    def _create_session_for_start(
        ...
        last_completed = self.storage.get_last_completed_round(resume_session_id)
    ...
    async def _run_session(
        ...
        try:
            ...
            while True:
                ...
                decision_result = await self.decision_engine.decide_with_prompt(spec, context, history, current_params)
    ...
    async def _finish_session(self, session_id: int, status: str, message: str) -> None:
        ...
        with suppress(Exception):
            ReportGenerator(self.storage).save_report(session_id, self.settings.artifacts_dir)
    ...
    def _build_live_prompt_preview(self) -> Optional[PromptPreview]:
        ...
        snapshot = self.storage.get_loop_snapshot()
        history = list(reversed(self.storage.list_rounds(snapshot.current_session_id))) if snapshot.current_session_id else []
        if not history:
        ...
        return self.decision_engine.build_prompt_preview(spec, context, history, current_params)
